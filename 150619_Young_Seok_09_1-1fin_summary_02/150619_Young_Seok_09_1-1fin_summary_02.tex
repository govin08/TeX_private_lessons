...
\(y\)축에 평행한 직선인 \(x=1\)은 \(y=mx+n\)꼴로 표현할 수 없기 때문이다.
...
\sol
공식 \(y-y_1=m(x-x_1)\)에 \(m=3\), \((x_1,y_1)=(4,1)\)을 대입하면 \(y-1=3(x-4)\)이다.
이것을 보기 좋게 정리하면 \(y=3x-11\)이 된다.
...
\ans{
(1) \(y=-2x\) (2) \(y=3x-14\) (3) \(y=2x+8\) (4) \(y=1\) (5) \(x=5\)
}
...
\sol
(1)
공식 \(y-y_1=\frac{y_2-y_1}{x_2-x_1}(x-x_1)\)에 \((x_1,y_1)=(3,-2)\), \((x_2,y_2)=(1,4)\)를 대입하면 \(y-(-2)=\frac{4-(-2)}{1-3}(x-3)\)이다.
...
하지만 좌표평면에 두 점을 찍고 두 점을 지나는 직선을 생각해보면 이 직선은 \(x=2\)가 되어야 함을 쉽게 알 수 있다.
...
(혹은, \(3\)에 역수를 취한 \(\frac13\)에 마이너스를 붙인 \(-\frac13\)이 새로 구할 직선의 기울기라고 해도 된다.)
...
\exam{}
직선 \(3x+4y+1\)에 평행하고 점 \((3,-2)\)까지의 거리가 \(2\)인 직선의 방정식을 구하여라.
...
%
\exam{}
원점과 \(x+y+c=0\) 사이의 거리가 \(\sqrt2\)일 때, 실수 \(c\)의 값을 구하여라.
...
이므로 \(d>r\)이다.
따라서 직선이 원으로부터 멀리 떨어져 있게 되어 교점이 존재하지 않는다(그림 \ref{x^2+y^2=2}).
...
이것을 정리하면
\(y=-3x\pm2\sqrt{10}\)이다(그림 \ref{x^2+y^2=4}).
...
\((x_1,y_1)=(2,1),(-1,2)\)
이다.
따라서 구하는 두 접선은 \(2x+y=5\)와 \(-x+2y=5\)이다.

\textbf{(공식 \(y=mx\pm r\sqrt{m^2+1}\)을 이용하는 방법)}\\
...
\begin{figure}[h]
\center
\includegraphics[width=0.4\textwidth]{2x-y+1=0}
\caption{\(2x-y+1=0\)를 \(y=x\)에 대해 대칭이동}
\label{2x-y+1=0}
\end{figure}
...
\begin{figure}[h]
\center
\includegraphics[width=0.4\textwidth]{axises}
\caption{\((x-3)^2+(y-2)^2=1\)를 \(x\)축, \(y\)축에 대해 대칭이동}
\label{axises}
\end{figure}
...
\(\overline{AP}=\overline{A'P}\)이므로 \(\overline{AP}+\overline{BP}\)의 최솟값을 구하는 대신, \(\overline{A'P}+\overline{BP}\)의 최솟값을 구하면 된다.
...
\caption{\(\overline{AP}+\overline{BP}\)의 최솟값}
\label{APBP}
\end{figure}
