...
\bigskip\bigskip
지금까지 각의 크기를 나타낼 때, 30\degree, 90\degree, \(120\)\degree와 같이 도(\degree)를 단위로 하는 육십분법을 사용하였다.
...
%
\prob{오른쪽 그림과 같이 반지름의 길이가 \(10\)이고 중심각의 크기가 \(45\)\degree인 부채꼴의 호의 길이 \(l\)과 넓이 \(S\)를 각각 구하여라.}\label{arc1}
\end{minipage}
...
%
\exam{\(\frac\pi3=60\degree\)의 삼각비를 구하여라.}\label{tratio2}
\noindent\begin{minipage}{.6\textwidth}
오른쪽 그림에서
...
\newpage
\item
\(r=1\)로 두고 \(A(1,0)\)를 시계반대방향으로 \(\frac32\pi(=270\degree)\)만큼 회전시키면 \(P=(0,-1)\)이다.
따라서
\par\noindent
\begin{minipage}{.5\textwidth}
\begin{talign*}
